...
//! ANativeWindow に対して描画を行う
//! ANativeWindow は ANativeWindow_fromSurface によって得られる Surface
//! Surface は SurfaceView の描画先、もしくはテクスチャから得られる
//---------------------------------------------------------------------------
class tTVPBasicDrawDevice : public tTVPDrawDevice
{
	...
	/*
	tjs_uint8*	WorkBuffer;
	tjs_uint WorkBufferWidth;	//!< 横幅
	tjs_uint WorkBufferHeight;	//!< 縦幅
	*/

	bool IsLockBuffer;
	ANativeWindow_Buffer BackBuffer;
...
private:
	...
	bool CreateWorkBuffer();
	void DestroyWorkBuffer();
